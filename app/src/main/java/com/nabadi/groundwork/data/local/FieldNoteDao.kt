...
interface FieldNoteDao {
    @Query(
        "SELECT * FROM field_notes WHERE sync_state != 'PENDING_DELETE' " +
            "AND (sync_state != 'FAILED' OR sync_failedOperation IS NULL " +
            "OR sync_failedOperation != 'DELETE') ORDER BY updatedAt DESC"
    )
    ...
    @Query(
        "SELECT * FROM field_notes WHERE id = :id AND sync_state != 'PENDING_DELETE' " +
            "AND (sync_state != 'FAILED' OR sync_failedOperation IS NULL " +
            "OR sync_failedOperation != 'DELETE')"
    )
